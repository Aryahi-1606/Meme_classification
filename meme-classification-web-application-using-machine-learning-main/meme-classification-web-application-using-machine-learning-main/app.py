...
def get_model():
    global rf_model, knc_model, etc_model
    global sgd_model, mnb_model, lr_model
    global vectorizer
...
def get_prediction(image_path):
    ...
    # 🚀 DEMO FIX: Handle strong negative keywords
    negative_keywords = ["sad", "depressed", "tired", "hate", "cry", "pain", "alone", "fail"]

    if any(word in cleaned_text for word in negative_keywords):
        return "Negative"
    ...
    # Predictions
    rf = rf_model.predict(image)
    knc = knc_model.predict(image)
    etc = etc_model.predict(image)
    sgd = sgd_model.predict(text_vector)
    mnb = mnb_model.predict(text_vector)
    lr = lr_model.predict(text_vector)
    ...
    answers = [rf, knc, etc, sgd, mnb, lr]
    print("Model Votes:", answers)
...
def predict():
    if request.method == "POST":
        ...
        prediction = prediction.upper().replace("_", " ")
